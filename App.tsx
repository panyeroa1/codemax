...
const App: React.FC = () => {
  ...
  const [activeModel, setActiveModel] = useState(localStorage.getItem('activeModel') || MODELS.CODEMAX_PRO);
  ...
  const [ollamaUrl, setOllamaUrl] = useState(localStorage.getItem('ollamaUrl') || 'http://127.0.0.1:11434');
  ...
  return (
    <div className="flex h-[100dvh] bg-white dark:bg-[#0e0e11] text-zinc-900 dark:text-[#d1d1d1] font-sans transition-colors duration-300">
      ...
      <aside className={`fixed md:relative z-50 h-full flex flex-col border-r border-zinc-200 dark:border-zinc-800 transition-all duration-300 bg-[#f7f7f8] dark:bg-[#0e0e11] ${sidebarOpen ? 'w-72 translate-x-0' : 'w-0 -translate-x-full md:translate-x-0 md:w-0 overflow-hidden'}`}>
        <div className="p-4 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setSidebarOpen(false)} className="p-1.5 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-6 transition-all" aria-label="Close sidebar" title="Close sidebar">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#0e0e11] relative">
        ...
        <header className="h-14 border-b border-zinc-100 dark:border-zinc-800/50 flex items-center px-4 md:px-6 shrink-0 z-30 justify-between">
          ...
          <div className="flex items-center space-x-2">
            <button className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-6 transition-all text-zinc-500" aria-label="Share chat" title="Share chat">
        ...
        {/* Messaging Interface */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-6 py-8 md:py-12 space-y-8 md:space-y-12 scrollbar-hide max-w-4xl mx-auto w-full">
          {messages.length === 0 && (
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in duration-500`}>
              <div className={`max-w-[100%] sm:max-w-[85%] space-y-4`}>
        ...
        {/* Input Dock */}
        <div className="px-4 md:px-8 pb-6 md:pb-10 shrink-0">
          <div className="max-w-4xl mx-auto">
            <div className="relative bg-[#f7f7f8] dark:bg-[#1c1c1f] border border-zinc-200 dark:border-zinc-800 rounded-[28px] p-5 shadow-[0_10px_60px_rgba(0,0,0,0.05)] dark:shadow-none transition-all focus-within:ring-1 focus-within:ring-zinc-400 dark:focus-within:ring-zinc-600 focus-within:bg-white dark:focus-within:bg-[#202024] focus-within:shadow-2xl">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-zinc-200 dark:border-zinc-800/30">
                ...
                <div className="flex items-center space-x-3">
                  <button onClick={() => fileInputRef.current?.click()} className="p-2.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" aria-label="Attach file" title="Attach file">
                    <PaperClipIcon className="w-5 h-5" />
                  </button>
                  <MicrophoneInput
                    onTranscriptChange={(text) => setInput(prev => prev + (prev ? ' ' : '') + text)}
                    className="p-2.5"
                  />
                  <button
                    onClick={() => handleSend()}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
          <div className="w-full max-w-lg bg-white dark:bg-[#1c1c1f] border border-zinc-200 dark:border-zinc-800 rounded-6 p-10 shadow-[0_32px_128px_rgba(0,0,0,0.6)]">
            <div className="flex items-center justify-between mb-10">
              ...
              <button onClick={() => setShowAdmin(false)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-6 transition-all" aria-label="Close settings"><XMarkIcon className="w-6 h-6" /></button>
            </div>
      ...
      <input type="file" ref={fileInputRef} className="hidden" accept="image/*,application/pdf" title="File upload" onChange={(e) => {
